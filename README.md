...
当前默认研究主题为 **Transformer 架构与大模型应用**，目标是做成一个 **能讲清楚、能跑起来、能快速切换主题与知识源** 的项目。
...
> 我做了一个 Transformer 方向的研究助手项目，底层用 LangGraph 管理状态流，用 LangChain 管理模型、工具和结构化输出。  
> 项目里用了按需 ReAct 的 Agentic RAG：先规划，再判断是否检索，检索后做相关性评估，不够相关时改写问题重试，并带循环上限控制。  
> 同时我还做了上下文管理和长期记忆，让系统更适合多轮研究场景与面试演示。  
> 整体复杂度控制在个人项目可驾驭范围内，但关键能力完整。
...
原因很简单：  
**面试个人项目最重要的是“讲得清楚 + 能跑 + 有取舍”，不是盲目堆复杂度。**
